...
public class RepositoryValidator {
	...
	public int validate() {
		Display.getDefault().syncExec(new Runnable() {
            public void run() {
            	String message = NLS.bind("The repository {0} seems missing or not valid. Do you want to create or update it now ?", new Object[] { name });
                MessageDialog dlg = new MessageDialog(null, "Repository Validation", null,
                		message, MessageDialog.QUESTION,
                		new String[] { UPDATE_LABEL, CREATE_LABEL, CANCEL_LABEL }, 0);
                userChoice = dlg.open();
            }
		});
