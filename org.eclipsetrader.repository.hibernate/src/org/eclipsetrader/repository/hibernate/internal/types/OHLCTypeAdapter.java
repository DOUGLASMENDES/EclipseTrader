...
public class OHLCTypeAdapter implements UserType {
	...
	public void nullSafeSet(PreparedStatement st, Object value, int index) throws HibernateException, SQLException {
		if (value == null) {
			...
			if (ohlc.getDate() == null) st.setNull(index++, Types.DATE); st.setDate(index++, new java.sql.Date(ohlc.getDate().getTime()));
			if (ohlc.getOpen() == null) st.setNull(index++, Types.DOUBLE); st.setDouble(index++, ohlc.getOpen());
			if (ohlc.getHigh() == null) st.setNull(index++, Types.DOUBLE); st.setDouble(index++, ohlc.getHigh());
			if (ohlc.getLow() == null) st.setNull(index++, Types.DOUBLE); st.setDouble(index++, ohlc.getLow());
			if (ohlc.getClose() == null) st.setNull(index++, Types.DOUBLE); st.setDouble(index++, ohlc.getClose());
			if (ohlc.getVolume() == null) st.setNull(index++, Types.INTEGER); st.setLong(index++, ohlc.getVolume());
		}
	}
