...
public class SecuritiesPage extends AbstractContributionPage {

	public SecuritiesPage() {
		super("Securities");
		setDescription("Individual security contribution");
	}
	...
	public void setInput(Backtest test) {
		...
		for (int i = 0; i < trades.length; i++) {
			ContributionStatistic c = (ContributionStatistic) stats.get(trades[i].getSecurity().toString());
			if (c == null) {
				c = new ContributionStatistic(trades[i].getSecurity().toString());
				stats.put(c.name, c);
			}
