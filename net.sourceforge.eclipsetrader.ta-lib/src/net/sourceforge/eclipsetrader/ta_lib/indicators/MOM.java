...
public class MOM extends Factory
{
    public static final String DEFAULT_LABEL = Messages.MOM_DefaultLabel;
    ...
    public IndicatorPlugin createIndicator()
    {
        IndicatorPlugin indicator = new TALibIndicatorPlugin() {
            ...
            public void calculate()
            {
                ...
                double[] outReal = getOutputArray(getBarData(), TALibPlugin.getCore().MOM_Lookback(period));
                
                TALibPlugin.getCore().MOM(startIdx, endIdx, inReal, period, outBegIdx, outNbElement, outReal);
    ...
    public IndicatorPluginPreferencePage createPreferencePage()
    {
        IndicatorPluginPreferencePage page = new TALibIndicatorPreferencePage() {
            ...
            public void createControl(Composite parent)
            {
                ...
                addColorSelector(content, "color", Messages.MOM_Color, DEFAULT_COLOR); //$NON-NLS-1$
                addLabelField(content, "label", Messages.MOM_Label, DEFAULT_LABEL); //$NON-NLS-1$
                addLineTypeSelector(content, "lineType", Messages.MOM_LineType, DEFAULT_LINETYPE); //$NON-NLS-1$
                addInputSelector(content, "input", Messages.MOM_Input, DEFAULT_INPUT, false); //$NON-NLS-1$
                addIntegerValueSelector(content, "period", Messages.MOM_Period, 1, 9999, DEFAULT_PERIOD); //$NON-NLS-1$
            }
        };
