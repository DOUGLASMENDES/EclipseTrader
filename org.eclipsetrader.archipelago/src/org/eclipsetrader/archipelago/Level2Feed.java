	...
	public void run() {
		...
		for (int i = 0; i < 5 && !stopping; i++) {
			try {
				...
				Status status = new Status(Status.ERROR, ArchipelagoPlugin.PLUGIN_ID, 0, "Error connecting to server", e);
		...
		while (!stopping) {
			try {
				...
				Status status = new Status(Status.ERROR, ArchipelagoPlugin.PLUGIN_ID, 0, "Error receiving stream", e);
				ArchipelagoPlugin.log(status);
				break;
			}
		}
