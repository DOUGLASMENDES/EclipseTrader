...
public class Util {
	...
	public static String getDateFormatPattern() {
		...
		if (format instanceof SimpleDateFormat) {
		    ...
		    if (pattern.indexOf("yyyy") == -1) //$NON-NLS-1$
		    	pattern = pattern.replaceAll("yy", "yyyy"); //$NON-NLS-1$ //$NON-NLS-2$
		    if (pattern.indexOf("d") != -1 && pattern.indexOf("dd") == -1) //$NON-NLS-1$ //$NON-NLS-2$
		    	pattern = pattern.replaceAll("d", "dd"); //$NON-NLS-1$ //$NON-NLS-2$
		    if (pattern.indexOf("MMM") != -1) //$NON-NLS-1$
		    	pattern = pattern.replaceAll("MMM", "MM"); //$NON-NLS-1$ //$NON-NLS-2$
		    else if (pattern.indexOf("M") != -1 && pattern.indexOf("MM") == -1) //$NON-NLS-1$ //$NON-NLS-2$
