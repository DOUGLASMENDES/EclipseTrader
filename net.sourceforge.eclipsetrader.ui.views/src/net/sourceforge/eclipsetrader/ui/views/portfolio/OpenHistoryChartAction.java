...
public class OpenHistoryChartAction implements IWorkbenchWindowActionDelegate, IViewActionDelegate
{
  ...
  public void run(IAction action) 
  {
    ...
    if (pg.getActivePart() instanceof PortfolioView)
    {
      ...
      if (data != null)
      {
        ...
        for (int i = 1;; i++)
        {
          IViewReference ref = page.findViewReference(HistoryChartView.VIEW_ID, String.valueOf(i));
          if (ref == null)
          {
            ViewsPlugin.getDefault().getPreferenceStore().setValue("chart." + String.valueOf(i), data.getSymbol()); //$NON-NLS-1$
            try {
              page.showView(HistoryChartView.VIEW_ID, String.valueOf(i), IWorkbenchPage.VIEW_ACTIVATE);
