...
import net.sourceforge.eclipsetrader.ui.views.trading.Book;
...
public class OpenLevel2Action implements IWorkbenchWindowActionDelegate, IViewActionDelegate
{
  ...
  public void run(IAction action) 
  {
    ...
    if (pg.getActivePart() instanceof PortfolioView)
    {
      ...
      if (data != null)
      {
        ...
        for (int i = 1;; i++)
        {
          IViewReference ref = page.findViewReference(Book.VIEW_ID, String.valueOf(i));
          if (ref == null)
          {
            ViewsPlugin.getDefault().getPreferenceStore().setValue("book." + String.valueOf(i), data.getSymbol()); //$NON-NLS-1$
            try {
              page.showView(Book.VIEW_ID, String.valueOf(i), IWorkbenchPage.VIEW_ACTIVATE);
