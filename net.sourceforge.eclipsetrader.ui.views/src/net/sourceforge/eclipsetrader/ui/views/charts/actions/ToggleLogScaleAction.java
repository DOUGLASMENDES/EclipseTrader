...
/**
 */
public class ToggleLogScaleAction implements IViewActionDelegate, IWorkbenchWindowActionDelegate
{
  ...
  public void run(IAction action)
  {
    ...
    if (pg.getActivePart() instanceof ChartView)
    {
      ...
      view.getSelectedZone().setLogScale(action.isChecked());
      for (int i = 0; i < list.size(); i++)
        ((IAction)list.get(i)).setChecked(action.isChecked());
    }
  }
  ...
  public void selectionChanged(IAction action, ISelection selection)
  {
    if (selection != null)
    {
      if (selection instanceof ChartSelection && selection.isEmpty() == false)
        action.setChecked(((ChartSelection)selection).getChartCanvas().isLogScale());
      else
