...
public class IntradayHistoryStoreTest extends TestCase {
    ...
    public void testPutHistoryData() throws Exception {
    	...
    	properties.setProperty(TimeSpan.minutes(1).toString(), new IOHLC[] { new OHLC(new Date(), 1.0, 2.0, 3.0, 4.0, 5L) });
    ...
    public void testPutHistoryDataWithDifferentTimeSpan() throws Exception {
    	IntradayHistoryStore store = new IntradayHistoryStore(null, repository);
    	StoreProperties properties = new StoreProperties();
    	properties.setProperty(TimeSpan.minutes(1).toString(), new IOHLC[] { new OHLC(new Date(), 1.0, 2.0, 3.0, 4.0, 5L), new OHLC(new Date(), 1.0, 2.0, 3.0, 4.0, 4L) });
    	properties.setProperty(TimeSpan.minutes(2).toString(), new IOHLC[] { new OHLC(new Date(), 1.0, 2.0, 3.0, 4.0, 9L) });
    	store.putProperties(properties, null);
    ...
    public void testDontReplaceExistingHistoryData() throws Exception {
    	...
    	properties.setProperty(TimeSpan.minutes(1).toString(), new IOHLC[] { ohlc1 });
    	store.putProperties(properties, null);
    	assertEquals(1, store.data.size());
    	HistoryData data = store.data.get(0);

    	properties.setProperty(TimeSpan.minutes(1).toString(), new IOHLC[] { ohlc1, ohlc2 });
    	store.putProperties(properties, null);
