...
/**
 */
public class BuyArrow extends ObjectPlugin
{
    private Date date;
    ...
    public void drawObject(GC gc, boolean selected)
    {
        if (date != null)
        {
            ...
            gc.drawImage(image, p1.x + location.x - imageData.width / 2, p1.y);
            String s = nf.format(getPlot().getScaler().convertToRoundedValue(p1.y));
            gc.drawString(s, p1.x + location.x - imageData.width / 2, p1.y - gc.stringExtent(s).y, true);
        }
    }
