...
public class Scaler {
	...
	private List<Double> scaleList = new ArrayList<Double>();
	...
	public List<Double> getScaleArray() {
		int ticks;
		...
		double interval = 0;
		int loop;
		for (loop = 0; loop < scaleList.size(); loop++) {
			interval = (scaleList.get(loop)).doubleValue();
