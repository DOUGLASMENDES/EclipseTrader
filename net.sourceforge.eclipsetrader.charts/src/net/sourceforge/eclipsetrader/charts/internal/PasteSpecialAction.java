...
public class PasteSpecialAction extends Action implements IPartListener
{
    ...
    public void run()
    {
        ...
        if (selection instanceof TabSelection)
        {
            ...
            tab.setLabel(indicator.getParent().getLabel());
            tab.getIndicators().add(indicator);
            
            ((TabSelection) selection).getChartTab().getParent().getTabs().add(tab);
