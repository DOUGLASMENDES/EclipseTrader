...
public class TradeSystemService implements ITradeSystemService, IAdapterFactory {
		...
		for (int j = 0; j < configElements.length; j++) {
			if (id.equals(configElements[j].getAttribute("id"))) { //$NON-NLS-1$
				try {
					...
					Status status = new Status(Status.WARNING, Activator.PLUGIN_ID, 0, "Unable to create strategy with id " + id, e);
	...
	private void saveRepository(File file) {
		try {
			...
			Status status = new Status(Status.ERROR, Activator.PLUGIN_ID, 0, "Error saving repository", e); //$NON-NLS-1$
			Activator.log(status);
		}
	}
}
