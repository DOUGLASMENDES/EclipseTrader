...
public class FibolineToolFactory implements IChartObjectFactory, IExecutableExtension {
    private String id;
    ...
    private FibolineToolObject object;

    public class FibolineToolObject implements IChartObject, IEditableChartObject {
    	private Point p1;
    	private Point p2;
        ...
        public boolean containsPoint(int x, int y) {
        	if (p1 != null && p2 != null) {
        		if (x >= p1.x && x <= p2.x) {
        			if (Math.abs(y - p1.y) <= 2 || Math.abs(y - p2.y) <= 2)
        				return true;
        			if (factorLine != null) {
        				for (int i = 0; i < factorLine.length; i++) {
        	    			if (Math.abs(y - factorLine[i]) <= 2)
        	    				return true;
        				}
        			}
        		}
        ...
        public boolean isOnEditHandle(int x, int y) {
    		if (p1 != null && p2 != null) {
    			if (Math.abs(x - p1.x) <= 2 && Math.abs(y - p1.y) <= 2)
    				return true;
    			if (Math.abs(x - p2.x) <= 2 && Math.abs(y - p1.y) <= 2)
    				return true;
    			if (Math.abs(x - p1.x) <= 2 && Math.abs(y - p2.y) <= 2)
    				return true;
    			if (Math.abs(x - p2.x) <= 2 && Math.abs(y - p2.y) <= 2)
    				return true;
    		}
    	    return false;
        }
    	...
    	protected void paintShape(IGraphics graphics) {
        	if (p1 != null && p2 != null) {
                ...
                graphics.drawString(s, p1.x, p1.y - p.y);
            	...
            	if (hasFocus()) {
            		...
            		graphics.fillRectangle(p2.x - 2, p1.y - 2, 5, 5);
            		graphics.fillRectangle(p1.x - 2, p2.y - 2, 5, 5);
	...
	public FibolineToolFactory() {
		object = new FibolineToolObject();
	}
