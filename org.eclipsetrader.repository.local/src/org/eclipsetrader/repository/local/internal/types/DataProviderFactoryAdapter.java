...
public class DataProviderFactoryAdapter extends XmlAdapter<String,IDataProviderFactory> {
    ...
    protected IDataProviderFactory getFactory(String id) {
		...
		for (int j = 0; j < configElements.length; j++) {
			...
			if (id.equals(strID)) {
				try {
					...
					Status status = new Status(Status.WARNING, Activator.PLUGIN_ID, 0, "Unable to create data provider factory with id " + id, e);
		...
		Status status = new Status(Status.WARNING, Activator.PLUGIN_ID, 0, "Failed to load provider " + id, null);
		Activator.getDefault().getLog().log(status);
